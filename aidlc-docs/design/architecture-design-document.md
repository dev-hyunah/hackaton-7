...
- **결정**: `AbstractAiEngine` 인터페이스로 AI 엔진 런타임 교체 가능
- **이유**: 외부 API 가용성·과금 상황에 따라 엔진을 교체할 수 있도록 추상화
- **결과**: `GroqAiEngine` (현재 활성), `ClaudeAiEngine`, `OllamaAiEngine`, `MockAiEngine` (Fallback) 동일 인터페이스
- **트레이드오프**: 추상화 계층 1개 추가 (무시 가능)
- **현재 상태**: **`GroqAiEngine` 활성 — Groq 무료 API로 `llama-3.3-70b-versatile` 실호출**
  - `GROQ_API_KEY` 환경변수 설정 필요 (로컬: `backend/.env`, AWS: App Runner 환경변수)
  - API 오류 또는 Key 미설정 시 `MockAiEngine`으로 자동 fallback
  - `ClaudeAiEngine.analyze_strategy()`: 구현 완료, `ANTHROPIC_API_KEY` 설정 시 교체 가능
  - `OllamaAiEngine`: 로컬 Ollama 서버 연동 구현 완료 (기본 모델: `exaone3.5:7.8b`)
...
│  │  │  │  GroqAiEngine   ──► 외부 Groq API (현재 활성)   │  │  │   │
│  │  │  │  ClaudeAiEngine ──► 외부 Anthropic API (미연결) │  │  │   │
│  │  │  │  OllamaAiEngine ──► 로컬 Ollama (로컬 전용)     │  │  │   │
...
│  Auth: GROQ_API_KEY (환경변수)  /  무료 티어                          │
│                                                                    │
│  Anthropic Claude API (미연결 — 대체 가능)                            │
...
│   │   └── AbstractAiEngine
│   │       ├── GroqAiEngine → Groq API (현재 활성)
│   │       ├── ClaudeAiEngine → Anthropic API (미연결)
│   │       ├── OllamaAiEngine → 로컬 Ollama
...
> **현재 동작**: `GroqAiEngine` 활성 — Groq 무료 API (`llama-3.3-70b-versatile`) 실호출 중.
> 오류 또는 `GROQ_API_KEY` 미설정 시 MockAiEngine으로 자동 fallback.

```
AbstractAiEngine (인터페이스)
      │
      ├── GroqAiEngine  ← 현재 활성 (AiRecommendationService에서 사용)
      │   ├── analyze_strategy(): 구현 완료 (GROQ_API_KEY 설정 시 활성화)
      │   │   ├── 프롬프트 구성 (노선·편명·출발일·등급별 현황·이슈 텍스트)
      │   │   ├── Groq API 호출 (llama-3.3-70b-versatile, response_format=json)
      │   │   ├── JSON 응답 파싱 (irrelevant 판정 + 4개 등급별 가격·근거)
      │   │   ├── BR-03 ±30% 클램핑 적용
      │   │   └── 요청/응답/소요시간/토큰 로깅 (ai_strategy.log)
      │   └── generate_recommendation(): MockAiEngine 위임
      │
      ├── ClaudeAiEngine  ← 대체 가능 (ANTHROPIC_API_KEY 설정 시)
      │   ├── analyze_strategy(): 구현 완료
      │   └── generate_recommendation(): MockAiEngine 위임
      │
      ├── OllamaAiEngine  ← 로컬 전용 (Ollama 서버 필요)
      │   ├── analyze_strategy(): 구현 완료 (기본 모델: exaone3.5:7.8b)
      │   └── generate_recommendation(): MockAiEngine 위임
      │
      └── MockAiEngine  ← Fallback (API 오류·Key 미설정 시 자동 전환)
          ├── 규칙 기반 추천 (현재가 ±10~20%)
          ├── L/F 기반 가중치 조정
          └── 노선·날짜 관련성 키워드 판단
```
